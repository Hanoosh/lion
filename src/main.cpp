...
#if defined(NDEBUG)
# error "Lion cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x00000968055154fcbf2b17f46e8ef608a938ea35adfca278ff348a8bd254b4b8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lion: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lion Signed Message:\n";
...
{
    // Lion: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 120; // Lion:2 minutes
static const int64 nTargetSpacing = 120; // Lion: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
        ...
        // LiteCoinX: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lion: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Lion: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000003c981ae65ec0e0d52ee8044824f0d5c69c350904ca085c100f4803c3d9d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "this shit drives me creazy!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 15;
        ...
        assert(block.hashMerkleRoot == uint256("0x1304625e71f019dac1c4507224af28b9b4d1b96b3ea667459d00d23dfa6b5859"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Lion: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LionMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LionMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LionMiner(CWallet *pwallet)
{
    printf("LionMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lion-miner");
    ...
    try { loop {
        ...
        printf("Running LionMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LionMiner, pwallet));
}
